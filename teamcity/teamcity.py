...
class TeamCity:
    def __init__(self, server=None, tokens=None, user=None, password=None, guest=False):
        self.server = server or os.environ.get('TEAMCITY_SERVER', None)
        ...
        self.authentication_method, self.base_url, self.header = self.check_auth_method(server)
    ...
    def request_base(self, url, method, extra_headers={}, data=None, timeout=60, retries=3):
        ...
        while retries > 0:
            try:
                ...
                logging.error(f'Failed to post request to TeamCity: {e}, retries left: {retries}')

            if response is None or response.status_code != 200:
                logging.error(f'Failed to post request to TeamCity: {response.status_code}, retries left: {retries}')
            else:
                return response
